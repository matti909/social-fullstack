...
export class ProfileComponent extends BaseComponent {
  ...
  private setIsAuthUserProfile() {
    this.isAuthUserProfile = !!(
      this.authUser && this.authUser.id == this.profileUser?.id
    );
  }
  ...
  setBio(): void {
    ...
    this.profileService
      ...
      .subscribe({
        next: (userResult) => {
          if (userResult) {
            this.handleSuccess(userResult, 'Your bio is updated.');
            this.bioInputValue = '';
          }
        },
  ...
  private handleSuccess(userResult: Partial<Maybe<User>>, message: string) {
    if (userResult) {
      ...
      if (this.authUser) {
        ...
        this.authService.storeUser(this.authUser as any);
      }
    }
  }
